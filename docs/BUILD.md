...
2. Configure vcpkg
	...
	* (Windows) From repo root in PowerShell, run the executable explicitly (so PowerShell does not treat "vcpkg" as a module):
	  .\vcpkg\vcpkg.exe install --overlay-triplets=custom-triplets '@vcpkg.txt' --triplet x86-windows-static
	  Or from inside vcpkg: .\vcpkg.exe install --overlay-triplets=../custom-triplets '@../vcpkg.txt' --triplet x86-windows-static
